...
function summarize(
  ...
  const evidence = alert.evidence as Record<
    string,
    unknown
  >;
...
export function AdminParliamentaryQueue({
  alerts
}: Props) {
  const rows = useMemo(
    () => alerts.map(summarize),
    [alerts]
  );
  ...
  return (
    <div className="admin-panel alerts-queue-panel parliamentary-queue">
      <div className="alerts-filter-header">
        <div>
          <p className="eyebrow">
            FILA POR PARLAMENTAR
          </p>
          <h2>
            {filtered.length} gabinete(s)
            exibido(s)
          </h2>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="parliamentary-queue-priority">
                ...
                <small>
                  {row.highPriorityCount} sinal(is)
                  de alta · {row.documentCount}{" "}
                  documento(s)
                </small>
                <time>
      ...
      {!filtered.length ? (
        <div className="empty-state">
          ...
          <p>
            Ajuste ou limpe os filtros da fila.
          </p>
        </div>
      ) : null}
    </div>
